...
function getConfig() {
    ...
    throw new Error(`Specify a ${configScriptFileName} or ${configJsonFileName} file to configure the swagen tool.`);
}
...
function processInputs(config) {
    for (let profileKey in config) {
        ...
        if (!profile.file && !profile.url) {
            throw new Error(`[${profileKey}] Must specify a file or url as the swagger input`);
        }
        if (!profile.output) {
            throw new Error(`[${profileKey}] Must specify an output file path.`);
        }
        if (!profile.debug) {
            profile.debug = {};
        }
