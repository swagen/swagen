    ...
    questions: [
        {
            when: answers => {
                console.log();
                console.log(`Generators are special NPM packages that swagen uses to generate code from a swagger definition.`);
                console.log(`Anybody can write a generator and publish it as an NPM package for others to use. Generator package names must be prefixed with 'swagen-'.`);
                console.log(`To use a generator, it must be installed globally on your system.`);
                console.log(`For the following question, specify the name of the generator you wish to use. You can specify just the part of the name after the 'swagen-' prefix.`);
            },
            type: 'input',
            name: 'generator',
